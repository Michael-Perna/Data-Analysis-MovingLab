...
class StatPlot():
    ...
    def plotHistAcc(dist,bins,weights,CDF,quantile,fname,receiver, do_xlim):
        # Plot Histogtam
        ...
        if do_xlim:
            xlim = quantile[2] + 1
            tick= np.sort(np.append(quantile.round(decimals=2),np.arange(0.5, int(xlim), 0.5)))
            ax.set_xticks(tick)
            ax.set_xlim(0,xlim)
        # Ax attributes
        ax.legend()
        ...
        ax.text(0.8,0.7,'Nombre d\'époques : %i \néquivalent à : %.0f de jours de navigations'%(len(dist),len(dist)/3600/24))
    ...
    def plot_HPE(df, df2, df3, receiver,receiver2,receiver3, posMode,posMode2,posMode3):
        ...
        ax.text(3600,44,'Récepteur : {} $\mu$ {:.2f}m et $\sigma$: {:.2f}m,\nRécepteur : {} $\mu$ {:.2f}m et $\sigma$ {:.2f}m,\nRécepteur : {} $\mu$ {:.2f}m et $\sigma$ {:.2f}m'.format(receiver,posMode['mean'],posMode['std'], receiver2,posMode2['mean'],posMode2['std'], receiver3,posMode3['mean'],posMode3['std'])
                ,fontsize=18,
        ...
        ax.set_title('Erreurs de postionnement lateral',fontsize=18,
                 ...
                 color='white')
        ax.set_ylabel('Erreur de Position Lateral [m]',fontsize=16)
        ax.set_xlabel('Ensemble des époques sans intéruptions [s] ',fontsize=16)
        ax.set_ylim(0,8)
        ax.grid(which='both', axis='y')

        ax.text(0,7,'Récepteur : {} $\mu$ {:.2f}m et $\sigma$: {:.2f}m,\nRécepteur : {} $\mu$ {:.2f}m et $\sigma$ {:.2f}m,'.format(receiver,posMode['mean'],posMode['std'], receiver2,posMode2['mean'],posMode2['std'])
